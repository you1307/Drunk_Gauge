...
public class SaveData {
    ...
    public void SaveAsteroidGameHighScore(Context context, String UserID, int NewScore) {
        FirebaseDatabase database = FirebaseDatabase.getInstance();
        DatabaseReference DataRef = database.getReference("Users/" + UserID + "/");

        SharedPreferences pref = context.getSharedPreferences("User", MODE_PRIVATE);
        int OldScore = pref.getInt("AsteroidScore", 1);

        if (NewScore > OldScore) {
            DataRef.child("AsteroidScore").setValue(NewScore);
            Log.v("AsteroidSave", "Saved: "+NewScore);
            SharedPreferences.Editor prefE = pref.edit();
            prefE.putInt("AsteroidScore", NewScore);
            prefE.apply();
        }
    }
